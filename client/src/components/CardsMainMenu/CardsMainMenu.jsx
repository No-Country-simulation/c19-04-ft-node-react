...
const CardsMainMenu = ({ _id, title, imgUrl, price, description, time }) => {
    ...
    return (
        <div className="bg-customGray-50 w-[180px] min-h-[177px] rounded-[10px] bg-customLight flex flex-col gap-[5px] pt-[50px] p-[10px] text-black shadow-custom-light relative">
            <div className={`fixed inset-0 z-50 flex items-end justify-center transition-transform duration-700 ease-in-out ${modalIsOpen ? 'translate-y-0' : 'translate-y-full'}`}>
                {modalIsOpen && (
                    <DetailModal isOpen={modalIsOpen} onClose={closeModal} product={product} />
                )}
            </div>
