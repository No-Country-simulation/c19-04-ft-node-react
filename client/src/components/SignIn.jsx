...
export default function SignIn() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!/^[a-zA-ZÀ-ÿ\u00f1\u00d1\s]+$/.test(formData.nombre)) {
      newErrors.nombre = 'El nombre solo puede contener letras, espacios y tildes';
    }
    ...
    if (!/^[a-zA-ZÀ-ÿ\u00f1\u00d1\s]+$/.test(formData.apellido)) {
      newErrors.apellido = 'El apellido solo puede contener letras, espacios y tildes';
    }
    ...
    if (!formData.contraseña.trim()) {
      ...
      !/^(?=.*[A-Z])(?=.*\d{2,}).{8,}$/.test(formData.contraseña)
    ) {
  ...
  return (
    <form onSubmit={handleSubmit} className='space-y-2'>
      <div>
        ...
        <input type="text" id="nombre" name="nombre" value={formData.nombre} onChange={handleChange} className='border border-black rounded-lg ml-2' />
        {errors.nombre && <span style={{ color: 'red' }}>{errors.nombre}</span>}
      </div>

      <div>
        <label htmlFor="apellido">Apellido:</label>
        <input type="text" id="apellido" name="apellido" value={formData.apellido} onChange={handleChange} className='border border-black rounded-lg ml-2' />
...
<div>
  ...
  {errors.terminosCondiciones && (
    <span style={{ color: 'red' }}>{errors.terminosCondiciones}</span>
      ...
      <button type="submit" className='bg-purple-400 rounded-lg p-2'>Enviar</button>
    </form>
  );
}
