...
const FilterFood = ({ categories }) => {
    ...
    return (
        <div className="flex place-content-center text-black">
            <ul className="flex flex-row overflow-y-auto  custom-scrollbar">
                {categories.map((item, index) => (
                    <li key={index} className="ml-1">
                        <button
                            onClick={(e) => handleChangeType(e, item)}
                            value={item}
                            className={`${filters.categoryFilter === item
                                    ? "bg-customGreen"
                                    : "bg-customLight"
                                } w-max px-4 h-[41px] text-[14px] font-medium rounded-[20px] hover:text-customRed-400 hover:underline hover:underline-offset-4 hover:decoration-customRed ease-linear duration-300`}
                        >
