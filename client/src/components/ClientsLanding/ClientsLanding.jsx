...
const ClientsLanding = () => {
    ...
    const items = [client1, client2, client3, client4, client5, client6, client7, client8]; // Tu lista de elementos
    const duplicatedItems = [...items, ...items, ...items]; // Duplicar los elementos
    return (
        <div className='flex flex-col justify-center items-center gap-4 '>
            <div className='text-center tracking-tighter max-w-96'>
                <h3 className='text-[40px] '>Nuestros Clientes</h3>
                <p className='text-xl'>Clientes satisfechos que han transformado sus operaciones</p>
            </div>

            <div className="flex flex-col gap-4 overflow-hidden">
                <div className="flex flex-row gap-3 py-1 marquee-left">
                    {duplicatedItems.map((item, index) => (
                        <CardClientLanding imgClient={item} key={index} />
                    ))}
                </div>
                <div className="flex flex-row gap-3 py-1 marquee-right">
                    {duplicatedItems.map((item, index) => (
                        <CardClientLanding imgClient={item} key={index} />
                    ))}
                </div>
